...
def profile():
    ...
    if session['roleid'] == '3':
        return render_template('profile.html', infos=dict['infos'][0], notifications=get_notifications_writer())
    else:
        return render_template('profile_Editor.html', infos=dict['infos'][0], notifications=get_notifications_editor())
...
def profileposts():
    ...
    if session['roleid'] == '3':
        return render_template('profilepost.html', posts=dict['submissions'], infos=dict2['infos'][0], notifications=get_notifications_writer())
    else:
        return render_template('profilepost_Editor.html', posts=dict['submissions'], infos=dict2['infos'][0], notifications=get_notifications_editor())
...
def delete_post():
    ...
    print write_id
    if type == 'Region':
        infos = requests.post('http://127.0.0.1:5000/api/editor/delete/region', json={"write_id": write_id, "username": session['user']})
    elif type == 'Destination':
        infos = requests.post('http://127.0.0.1:5000/api/editor/delete/destination', json={"write_id": write_id, "username": session['user']})
    else:
        infos = requests.post('http://127.0.0.1:5000/api/editor/delete/attraction', json={"write_id": write_id, "username": session['user']})
    return redirect(url_for('editor', pagenum=1))
...
@server.route('/profile/activity_logs', methods=['GET']) #changed
def activity_logs():
    acts = requests.get('http://127.0.0.1:5000/api/activity_logs',json={"username": session['user']})
    infos = requests.get('http://127.0.0.1:5000/api/profile',json={"username": session['user']})
    dict = json.loads(acts.text)
    dict2 = json.loads(infos.text)

    if session['roleid'] == '3':
        return render_template('activitylogs.html', acts=dict['acts'], infos=dict2['infos'][0],  notifications=get_notifications_writer())
    else:
        return render_template('activitylogs_Editor.html', acts=dict['acts'], infos=dict2['infos'][0], notifications=get_notifications_editor())
